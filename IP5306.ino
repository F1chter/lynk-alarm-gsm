...
#define IP5306_SetPowerOnLoadEnabled(v)         ip5306_set_bits(IP5306_REG_SYS_0, 2, 1, v)//0:dis,*1:en //eco mode
...
#define IP5306_SetBoostAfterVin(v)              ip5306_set_bits(IP5306_REG_SYS_1, 2, 1, v)//0:Disable, *1:Enable // after Vin off
...
void printIP5306Stats(){
    ...
    bool outLoad = IP5306_GetOutputLoad();
    uint8_t leds = IP5306_GetLevelLeds();
    Serial.printf("IP5306: Power Source: %s, Battery State: %s, Load: %s, Battery Available: %u%%\n", usb?"USB":"BATTERY", full?"FULL CHARGED":(usb?"CHARGING":"DISCHARGING"), outLoad?"high load":"no/light load", IP5306_LEDS2PCT(leds));
}
...
void printIP5306Settings(){
    ...
    Serial.printf("  BoostAfterVinOff: %s\n", IP5306_GetBoostAfterVin()?"Enable":"Disable");
